...
std::string Planet::GetName(std::string in_string)  {
...
std::string Planet::GetDate(std::string in_string)  {
...
double Planet::GetRadius(std::string in_string)  {
...
//Функция для вывода планет с определенной площадью
void Planet::AreaRange(float min_area, float max_area, std::vector<Planet> planet_arr)  {
...
//Функция для сортировки планет по дате или площади
void Planet::sortPlanets(std::vector<Planet>& vec, SortOptions choice)
{
...
void Planet::Print(std::ostream& output) const 
{
